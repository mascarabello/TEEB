...
drop_table <- "DROP TABLE IF EXISTS public.teeb_raiox_recuperacao_finalv2"
executeQuery(connec_local, drop_table)

createTableQuery <-  
  "CREATE TABLE public.teeb_raiox_recuperacao_finalv2 (
...
# Criando índices
indexQueries <- c(
  "CREATE INDEX teeb_raiox_recuperacao_finalv2_id_idx ON public.teeb_raiox_recuperacao_finalv2 USING btree (id)",
  "CREATE INDEX teeb_raiox_recuperacao_finalv2_idcar_imaflora_idx ON public.teeb_raiox_recuperacao_finalv2 USING btree (idcar_imaflora)",
  "CREATE INDEX teeb_raiox_recuperacao_finalv2_municipios_idx ON public.teeb_raiox_recuperacao_finalv2 USING btree (municipios)",
  "CREATE INDEX teeb_raiox_recuperacao_finalv2_prd_idx ON public.teeb_raiox_recuperacao_finalv2 USING btree (prd)")
...
#  dt <- subset(dt, idcar_imaflora > 0)
  ...
  dt$prd <- ifelse(dt$pastagem_2010 %in% c(1, 2) & 
                   dt$pastagem_2020 %in% c(1, 2) & (dt$pastagem_2011 == 3 | dt$pastagem_2012 == 3|
                                                      dt$pastagem_2013 == 3 | dt$pastagem_2014 == 3| 
                                                      dt$pastagem_2015 == 3 | dt$pastagem_2016 == 3 |
                                                      dt$pastagem_2017 == 3 | dt$pastagem_2018 == 3 |
                                                      dt$pastagem_2019 == 3),TRUE,FALSE)

  x <- dt %>%
    group_by(idcar_imaflora,municipios,prd) %>%
    summarise(rpd0910 = sum(rpd0910, na.rm = T),
  ...
  dbWriteTable(connec_local, 'teeb_raiox_recuperacao_finalv2', x, row.names = F, append = T)
