...
uso2010 <- raster("pa_br_usoterra_mapbiomas7_lapig_100m_2010_albers_comp.tif"); uso2010;
...
system.time(for (i in 1:bss$n) {
  ...
  dt$pastagem <- ifelse(dt$uso2021 %in% c(15), dt$uso2010,1000)
  dt$agricultura <- ifelse(dt$uso2021 %in% c(20, 39, 40, 41, 46, 47, 48, 62), dt$uso2010,1000)
  dt$mosaico <- ifelse(dt$uso2021 %in% c(21), dt$uso2010,1000)
  dt$silvicultura <- ifelse(dt$uso2021 %in% c(9), dt$uso2010,1000)
...
write.table(y_final_pastagem1, "/Users/marlucescarabello/Dropbox/Work/GPP/Teeb/P4_adicional/tabelas/output/uso_hist_2010_2021_pastagem_clean.csv",row.names = F, sep = ";")
...
write.table(y_final_agricultura, "/Users/marlucescarabello/Dropbox/Work/GPP/Teeb/P4_adicional/tabelas/output/uso_hist_2010_2021_agricultura.csv",row.names = F, sep = ";")

y_final_agricultura1 <- y_final_agricultura %>%  filter(classe != 'NAO')

write.table(y_final_agricultura1, "/Users/marlucescarabello/Dropbox/Work/GPP/Teeb/P4_adicional/tabelas/output/uso_hist_2010_2021_agricultura_clean.csv",row.names = F, sep = ";")
...
y <- data.frame(municipios = NA, uso2010 = NA, uso2021 = NA, uso2010ori = NA,uso2021ori = NA, area_ha = NA)
...
system.time(for (i in 1:bss$n) {
  ...
  x <- dt %>%
    group_by(municipios,uso2010,uso2021,uso2010ori,uso2021ori) %>%
    summarise(area_ha = n()*1.0) %>% as_tibble()
...
y_2010 <- y %>% 
  ...
  mutate(classe = ifelse(uso2010 %in% c(15), 'Pastagem',
                         ifelse(uso2010 %in% c(20, 39, 40, 41, 46, 47, 48, 62), 'Agricultura',
                                ifelse(uso2010 %in% c(9), 'Silvicultura',
                                       ifelse(uso2010 %in% c(21), 'Mosaico',
                                              ifelse(uso2010  %in% c(3, 4, 5, 11, 13, 49, 50), 'Vegetacao Nativa',
                                       ifelse(uso2010  %in% c(0, 12, 23, 24, 25, 29, 30, 31, 32, 33), 'Outros','ERRO'))))))) %>%
...
y_2010ori <- y %>% 
  ...
  mutate(classe = ifelse(uso2010ori %in% c(15), 'Pastagem',
                         ifelse(uso2010ori %in% c(9, 20, 21, 39, 40, 41, 46, 47, 48, 62), 'Agricultura',
...
y_2021v2 <- y %>% 
  filter(!is.na(municipios))%>% 
  mutate(classe = ifelse(uso2021 %in% c(15), 'Pastagem',
                         ifelse(uso2021 %in% c(9), 'Agri-Silvicultura',
                                ifelse(uso2021 %in% c(20), 'Agri-Cana',
                                       ifelse(uso2021 %in% c(21), 'Agri-Mosaico',
                                              ifelse(uso2021 %in% c(39), 'Agri-Soja',
                                                     ifelse(uso2021 %in% c(40), 'Agri-Arroz',
                                                            ifelse(uso2021 %in% c(41), 'Agri-OLavourasTemp',
                                                                   ifelse(uso2021 %in% c(46), 'Agri-Cafe',
                                                                          ifelse(uso2021 %in% c(47), 'Agri-Citrus',
                                                                                 ifelse(uso2021 %in% c(48), 'Agri-OLavurasPerene',
                                                                                        ifelse(uso2021 %in% c(62), 'Agri-Algodao',
                                                                                               ifelse(uso2021  %in% c(3, 4, 5, 11, 13, 49, 50), 'Vegetacao Nativa',
                                                                                                      ifelse(uso2021  %in% c(0, 12, 23, 24, 25, 29, 30, 31, 32, 33), 'Outros','ERRO')))))))))))))) %>%
                                                                            group_by(classe) %>% 
                                                                            summarise(area_final = sum(area_ha,na.rm = TRUE))
...
y_2021oriv2 <- y %>% 
  filter(!is.na(municipios))%>% 
  mutate(classe = ifelse(uso2021ori %in% c(15), 'Pastagem',
                         ifelse(uso2021ori %in% c(9), 'Agri-Silvicultura',
                                ifelse(uso2021ori %in% c(20), 'Agri-Cana',
                                       ifelse(uso2021ori %in% c(21), 'Agri-Mosaico',
                                              ifelse(uso2021ori %in% c(39), 'Agri-Soja',
                                                     ifelse(uso2021ori %in% c(40), 'Agri-Arroz',
                                                            ifelse(uso2021ori %in% c(41), 'Agri-OLavourasTemp',
                                                                   ifelse(uso2021ori %in% c(46), 'Agri-Cafe',
                                                                          ifelse(uso2021ori %in% c(47), 'Agri-Citrus',
                                                                                 ifelse(uso2021ori %in% c(48), 'Agri-OLavurasPerene',
                                                                                        ifelse(uso2021ori %in% c(62), 'Agri-Algodao',
                                                                                               ifelse(uso2021ori  %in% c(3, 4, 5, 11, 13, 49, 50), 'Vegetacao Nativa',
                                                                                                      ifelse(uso2021ori  %in% c(0, 12, 23, 24, 25, 29, 30, 31, 32, 33), 'Outros','ERRO')))))))))))))) %>%
                                                                            group_by(classe) %>% 
                                                                            summarise(area_final = sum(area_ha,na.rm = TRUE))
...
uso2010 <- raster("pa_br_usoterra_mapbiomas7_lapig_100m_2010_albers_comp.tif"); uso2010;
uso2021 <- raster("pa_br_usoterra_mapbiomas7_lapig_100m_2021_albers_comp.tif"); uso2021;
out <- raster("pa_br_usoterra_mapbiomas7_lapig_100m_2010_albers_comp.tif")

# Marking pasture pixels in LAPIG as pasture in MAPBIOMAS
bss <- blockSize(uso2010)
bss
lap <- readStart(uso2021)
uso2010 <- readStart(uso2010)
out <- readStart(out)
#out <- writeStart(out, filename = "pa_br_usoterra_mapbiomas7_lapig_100m_2010_albers.tif",format = 'Gtiff',overwrite=TRUE)
out <- writeStart(out, filename = "pa_br_usoterra_mapbiomas7_lapig_100m_2010_albers_comp_pastatual.tif",format = 'Gtiff',overwrite=TRUE,options=c("COMPRESS=DEFLATE","ZLEVEL=9","PREDICTOR=2","TFW=YES"))


for(i in 1:bss$n) {
  x <- getValues(uso2021, row = bss$row[i], nrows = bss$nrows[i])
  y <- getValues(uso2010, row = bss$row[i], nrows = bss$nrows[i])
  y[x[] != 15] = 1000
...
out <- readStart(out)
#out <- writeStart(out, filename = "pa_br_usoterra_mapbiomas7_lapig_100m_2010_albers.tif",format = 'Gtiff',overwrite=TRUE)
out <- writeStart(out, filename = "reclassificacao_agri/pa_br_usoterra_mapbiomas7_lapig_100m_2010_albers_comp_agriatual.tif",format = 'Gtiff',overwrite=TRUE,options=c("COMPRESS=DEFLATE","ZLEVEL=9","PREDICTOR=2","TFW=YES"))
