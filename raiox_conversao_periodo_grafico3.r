...
FROM public.teeb_raiox_conversao_final_2010_2020v3 as a
...
sum(acum_cpd)/1000000 as area_cpd_ha
FROM foo
...
#ggsave(filename = '/Users/marlucescarabello/Dropbox/Work/GPP/Teeb/P4_adicional/conversao_cpd_periodo_numero.png', plot = p1, units = 'in', dpi = 300, scale = 1.5)
ggsave(filename = '/Users/marlucescarabello/Dropbox/Work/GPP/Teeb/P4_adicional/mapas/reclassificacao/slide7_numero.pdf', plot = p1, units = 'in', dpi = 300, scale = 0.6)
#ggsave(filename = '/Users/marlucescarabello/Dropbox/Work/GPP/Teeb/P4_adicional/mapas/reclassificacao/slide7_numero_sn.pdf', plot = p1, units = 'in', dpi = 300, scale = 0.6)
 
## ÁREA
dt2 <- dt[c(1,3)]
dt2$perc <- dt$area_cpd_ha/sum(dt$area_cpd_ha)


dt2_label <- 
  dt2 %>% 
  mutate(perc = area_cpd_ha/ sum(area_cpd_ha)) %>% 
  mutate(labels = scales::percent(perc)) %>% 
  arrange(desc(tamanho)) %>% ## arrange in the order of the legend
  mutate(text_y = cumsum(area_cpd_ha) - area_cpd_ha/2) ### calculate where to place the text labels
...
ggsave(filename = '/Users/marlucescarabello/Dropbox/Work/GPP/Teeb/P4_adicional/mapas/reclassificacao/slide7_aarea_sn.pdf', plot = p2, units = 'in', dpi = 300, scale = 0.6)
